...
def test_set_get_position_should_not_change_value_if_pos_is_invalid():
    global droneSim
    expected_pos = ['100.0','100.0']
    droneSim.setPosition([10.0,10.0])
    droneSim.setPosition(['aaa', 'aaa'])
    actual_pos = droneSim.getPosition()
    assert expected_pos == actual_pos

def test_set_get_sensors():
    global droneSim
    expected_sensors = ['1.0','0.5','1.0','0.5']
...
def test_set_get_sensors_should_not_change_value_if_array_is_none():
    global droneSim
    expected_sensors = ['1.0','1.0','1.0','1.0']
    droneSim.setSensors([10.0,10.0,10.0,10.0])
    droneSim.setSensors(None)
    actual_sensors = droneSim.getSensors()
    assert expected_sensors == actual_sensors

def test_set_get_sensors_should_not_change_value_if_array_is_invalid():
    global droneSim
    expected_sensors = ['1.0','1.0','1.0','1.0']
    droneSim.setSensors([10.0,10.0,10.0,10.0])
    droneSim.setSensors(['aaa', 'aaa', 'aaa', 'aaa'])
    actual_sensors = droneSim.getSensors()
    assert expected_sensors == actual_sensors
